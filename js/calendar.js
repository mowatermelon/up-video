(function () {
	...
	//@param：为方法时需要传递的参数
	$.fn.extend({
		fullYearPicker: function (_config, param) {
			...
			return this
				...
				.each(
					function () {
						const me = $(this), year = _config.year || new Date().getFullYear(), newConifg = {
							...
							.on('click',
								function (e, setYear) {
									if (setYear)
										...
										$(this).children("a").hasClass('chevron-left') ? year-- : year++;
									setYearMenu(year);
									renderYear(
										year,
	...
	let month_arry;
	let week_arry;
	let month_cn = new Array("一月", "二月", "三月", "四月", "五月", "六月", "七月", "八月", "九月", "十月", "十一月", "十二月");//月
	let month_en = new Array("January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December");//月
	let week_cn = new Array("日", "一", "二", "三", "四", "五", "六");//星期
	let week_en = new Array("Su", "Mo", "Tu", "We", "Th", "Fr", "Sa");//星期
	 ...
	 * @returns {Boolean} res 当前日期是否超出边界
	 */
	...
	function checkBoundaryDate(data, page) {
	...
	window.renderListDate = function (data, page) {
		...
		!data && (data = $.data(document.body, "findData"))
		...
		$.each(data, function (index, item) {
			...
			$item.addClass(config.PUBLISH_CLASS).on('click', function (e) {
				...
				console.log(currPublishData);
				mdui.alert(window.getPublishDateStr(storeData[currDate]), currDate)
			});
			renderCountByUser(countData);
		});
